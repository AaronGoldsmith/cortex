...
- `cortex ingest` — ingest session history from AI tools. Flags: `--source` (Codex|goose|all), `--memory`, `--subagents`, `--all`, `--backfill-turns`
...
- **eval-auditor** (`.Codex/agents/eval-auditor.md`): If eval results show low relevance or precision, spawn this agent to diagnose failures, fix eval cases, add missing knowledge, and adjust confidence scores.
...
cortex/
  ...
  ingest.py     — Orchestrator + Codex-specific memory/subagent ingest
  ...
  providers/
    ...
    Codex.py    — Codex history.jsonl provider
